...
public:
  ...
  LAStools::BOOL manage_cell(const U32 cell_index, const LAStools::BOOL finalize=FALSE);

  // map points to cells
  LAStools::BOOL inside(const F64 x, const F64 y) const;
  U32 get_cell_index(const F64 x, const F64 y) const;

  // map cells to coarser cells
  LAStools::BOOL coarsen(const I32 cell_index, I32* coarser_cell_index, U32* num_cell_indices, I32** cell_indices) const;
  ...
  // iterate over cells
  LAStools::BOOL get_all_cells();
  LAStools::BOOL get_intersected_cells();
  LAStools::BOOL has_more_cells();

  // for LASquadtree
  LAStools::BOOL setup(F64 bb_min_x, F64 bb_max_x, F64 bb_min_y, F64 bb_max_y, F32 cell_size = 1000.0f);
  LAStools::BOOL setup(F64 bb_min_x, F64 bb_max_x, F64 bb_min_y, F64 bb_max_y, F32 cell_size, F32 offset_x, F32 offset_y);
  LAStools::BOOL tiling_setup(F32 min_x, F32 max_x, F32 min_y, F32 max_y, U32 levels);
  LAStools::BOOL subtiling_setup(F32 min_x, F32 max_x, F32 min_y, F32 max_y, U32 sub_level, U32 sub_level_index, U32 levels);
  ...
  U32* raster_occupancy(LAStools::BOOL(*does_cell_exist)(I32), U32 level) const;
  U32* raster_occupancy(LAStools::BOOL(*does_cell_exist)(I32)) const;
...
private:
  ...
  LAStools::BOOL intersect_circle_with_rectangle(const F64 center_x, const F64 center_y, const F64 radius, const F32 r_min_x, const F32 r_max_x, const F32 r_min_y, const F32 r_max_y);
  void raster_occupancy(LAStools::BOOL(*does_cell_exist)(I32), U32* data, U32 min_x, U32 min_y, U32 level_index, U32 level, U32 stop_level) const;
  void* current_cells;
  U32 next_cell_index;
};
