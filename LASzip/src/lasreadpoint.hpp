...
public:
  ...
  LAStools::BOOL setup(const U32 num_items, const LASitem* items, const LASzip* laszip=0);

  LAStools::BOOL init(ByteStreamIn* instream);
  LAStools::BOOL seek(const U32 current, const U32 target);
  LAStools::BOOL read(U8* const * point);
  LAStools::BOOL check_end();
  LAStools::BOOL done();
...
private:
  ...
  LAStools::BOOL layered_las14_compression;
  // used for chunking
  U32 chunk_size;
  U32 chunk_count;
  U32 current_chunk;
  ...
  I64* chunk_starts;
  U32* chunk_totals;
  LAStools::BOOL init_dec();
  LAStools::BOOL read_chunk_table();
