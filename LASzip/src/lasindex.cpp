...
LAStools::BOOL LASindex::add(const F64 x, const F64 y, const U32 p_index)
{
  I32 cell = spatial->get_cell_index(x, y);
  return interval->add(p_index, cell);
}

void LASindex::complete(U32 minimum_points, I32 maximum_intervals, const LAStools::BOOL verbose)
{
  if (verbose)
  {
...
void LASindex::print(LAStools::BOOL verbose)
{
  U32 total_cells = 0;
  U32 total_full = 0;
  U32 total_total = 0;
...
LAStools::BOOL LASindex::intersect_rectangle(const F64 r_min_x, const F64 r_min_y, const F64 r_max_x, const F64 r_max_y)
{
  have_interval = FALSE;
...
LAStools::BOOL LASindex::intersect_tile(const F32 ll_x, const F32 ll_y, const F32 size)
{
  have_interval = FALSE;
...
LAStools::BOOL LASindex::intersect_circle(const F64 center_x, const F64 center_y, const F64 radius)
{
  have_interval = FALSE;
...
LAStools::BOOL LASindex::get_intervals()
{
  have_interval = FALSE;
  return interval->get_merged_cell();
}

LAStools::BOOL LASindex::has_intervals()
{
...
LAStools::BOOL LASindex::read(FILE* file)
{
...
LAStools::BOOL LASindex::write(FILE* file) const
{
  if (file == 0) return FALSE;
...
LAStools::BOOL LASindex::read(const char* file_name)
{
...
LAStools::BOOL LASindex::append(const char* file_name) const
{
#ifdef LASZIPDLL_EXPORTS
  return FALSE;
#else
...
LAStools::BOOL LASindex::write(const char* file_name) const
{
  if (file_name == 0) return FALSE;
  char* name = strdup(file_name);
  if (strstr(file_name, ".las") || strstr(file_name, ".laz"))
...
LAStools::BOOL LASindex::read(ByteStreamIn* stream)
{
  if (spatial)
  {
    delete spatial;
...
LAStools::BOOL LASindex::write(ByteStreamOut* stream) const
{
...
#ifdef LASZIPDLL_EXPORTS
LAStools::BOOL LASindex::seek_next(LASreadPoint* reader, I64 &p_count)
{
  if (!have_interval)
  {
...
#else
LAStools::BOOL LASindex::seek_next(LASreader* lasreader)
{
  if (!have_interval)
  {
    if (!has_intervals()) return FALSE;
...
LAStools::BOOL LASindex::merge_intervals()
{
...
//    fprintf(stderr,"LASindex: used %d cells of total %d\n", used_cells, interval->get_number_cells());
    if (used_cells)
    {
      LAStools::BOOL r = interval->merge();
      full = interval->full;
