...
public:
  ...
  virtual LAStools::BOOL write_point(const LASpoint* point) = 0;
  virtual void update_inventory(const LASpoint* point) { inventory.add(point); };
  virtual LAStools::BOOL chunk() = 0;

  virtual LAStools::BOOL update_header(const LASheader* header, LAStools::BOOL use_inventory=FALSE, LAStools::BOOL update_extra_bytes=FALSE) = 0;
  virtual I64 close(LAStools::BOOL update_npoints=TRUE) = 0;
...
public:
  ...
  LAStools::BOOL set_directory(const CHAR* directory);
  void set_file_name(const CHAR* file_name);
  void set_appendix(const CHAR* appendix);
  void set_cut(U32 cut);
  void set_native(LAStools::BOOL native);
  LAStools::BOOL set_format(I32 format);
  LAStools::BOOL set_format(const CHAR* format);
  void set_force(LAStools::BOOL force);
  ...
  U32 get_cut() const;
  LAStools::BOOL get_native() const;
  LAStools::BOOL format_was_specified() const;
...
private:
  ...
  F32 scale_rgb;
  U32 format;
  LAStools::BOOL specified;
  LAStools::BOOL force;
  LAStools::BOOL native;
  U32 chunk_size;
  LAStools::BOOL use_stdout;
  LAStools::BOOL use_nil;
  LAStools::BOOL buffered;
};
