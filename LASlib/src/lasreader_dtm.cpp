...
LAStools::BOOL LASreaderDTM::open(const CHAR* file_name)
{
  if (file_name == 0)
  {
...
LAStools::BOOL LASreaderDTM::seek(const I64 p_index)
{
  return FALSE;
}

LAStools::BOOL LASreaderDTM::read_point_default()
{
...
void LASreaderDTM::close(LAStools::BOOL close_stream)
{
  if (file)
  {
    fclose(file);
...
LAStools::BOOL LASreaderDTM::reopen(const CHAR* file_name)
{
  if (file_name == 0)
  {
    fprintf(stderr,"ERROR: file name pointer is zero\n");
...
LAStools::BOOL LASreaderDTMrescale::open(const CHAR* file_name)
{
...
LAStools::BOOL LASreaderDTMreoffset::open(const CHAR* file_name)
{
...
LAStools::BOOL LASreaderDTMrescalereoffset::open(const CHAR* file_name)
{
  LASreaderDTM::set_scale_factor(scale_factor);
  LASreaderDTM::set_offset(offset);
  if (!LASreaderDTM::open(file_name)) return FALSE;
