...
void LASreaderBuffered::set_populate_header(LAStools::BOOL populate_header)
{
  lasreadopener.set_populate_header(populate_header);
  lasreadopener_neighbors.set_populate_header(populate_header);
}

LAStools::BOOL LASreaderBuffered::set_file_name(const char* file_name)
{
...
LAStools::BOOL LASreaderBuffered::add_neighbor_file_name(const char* file_name)
{
  // do we have a file name
  if (file_name == 0)
  {
...
LAStools::BOOL LASreaderBuffered::open()
{
...
LAStools::BOOL LASreaderBuffered::reopen()
{
  p_count = 0;
  point_count = 0;
  if (lasreader)
...
LAStools::BOOL LASreaderBuffered::remove_buffer()
{
  clean_buffer();
...
LAStools::BOOL LASreaderBuffered::inside_tile(const F32 ll_x, const F32 ll_y, const F32 size)
{
  inside = 1;
  t_ll_x = ll_x;
  t_ll_y = ll_y;
...
LAStools::BOOL LASreaderBuffered::inside_circle(const F64 center_x, const F64 center_y, const F64 radius)
{
  inside = 2;
  c_center_x = center_x;
  c_center_y = center_y;
...
LAStools::BOOL LASreaderBuffered::inside_rectangle(const F64 min_x, const F64 min_y, const F64 max_x, const F64 max_y)
{
  inside = 3;
  r_min_x = min_x;
  r_min_y = min_y;
...
LAStools::BOOL LASreaderBuffered::read_point_default()
{
  while (true)
  {
...
void LASreaderBuffered::close(LAStools::BOOL close_stream)
{
  if (lasreader) 
  {
...
LAStools::BOOL LASreaderBuffered::copy_point_to_buffer()
{
...
LAStools::BOOL LASreaderBuffered::copy_point_from_buffer()
{
