...
LAStools::BOOL LASreaderMerged::add_file_name(const char* file_name)
{
...
void LASreaderMerged::set_apply_file_source_ID(LAStools::BOOL apply_file_source_ID)
{
...
void LASreaderMerged::set_populate_header(LAStools::BOOL populate_header)
{
  this->populate_header = populate_header;
}

void LASreaderMerged::set_keep_lastiling(LAStools::BOOL keep_lastiling)
{
  this->keep_lastiling = keep_lastiling;
}

LAStools::BOOL LASreaderMerged::open()
{
  ...
  U32 i,j;
  LAStools::BOOL first = TRUE;
  LAStools::BOOL attributes = FALSE;
...
LAStools::BOOL LASreaderMerged::inside_tile(const F32 ll_x, const F32 ll_y, const F32 size)
{
  inside = 1;
  t_ll_x = ll_x;
  t_ll_y = ll_y;
...
LAStools::BOOL LASreaderMerged::inside_circle(const F64 center_x, const F64 center_y, const F64 radius)
{
  inside = 2;
  c_center_x = center_x;
  c_center_y = center_y;
...
LAStools::BOOL LASreaderMerged::inside_rectangle(const F64 min_x, const F64 min_y, const F64 max_x, const F64 max_y)
{
  inside = 3;
  r_min_x = min_x;
  r_min_y = min_y;
...
LAStools::BOOL LASreaderMerged::read_point_default()
{
...
void LASreaderMerged::close(LAStools::BOOL close_stream)
{
  if (lasreader) 
  {
...
LAStools::BOOL LASreaderMerged::reopen()
{
  p_count = 0;
  file_name_current = 0;
...
LAStools::BOOL LASreaderMerged::open_next_file()
{
