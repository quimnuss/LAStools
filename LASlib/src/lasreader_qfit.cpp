...
LAStools::BOOL LASreaderQFIT::open(const char* file_name)
{
  if (file_name == 0)
  {
...
LAStools::BOOL LASreaderQFIT::open(ByteStreamIn* stream)
{
  U32 i;
...
LAStools::BOOL LASreaderQFIT::seek(const I64 p_index)
{
  if (p_index < npoints)
  {
    p_count = p_index;
...
LAStools::BOOL LASreaderQFIT::read_point_default()
{
  if (p_count < npoints)
  {
    try { stream->getBytes((U8*)buffer, version); } catch(...)
...
void LASreaderQFIT::close(LAStools::BOOL close_stream)
{
  if (close_stream)
  {
    if (stream)
...
LAStools::BOOL LASreaderQFIT::reopen(const char* file_name)
{
  if (file_name == 0)
  {
    fprintf(stderr,"ERROR: file name pointer is zero\n");
...
LAStools::BOOL LASreaderQFITrescale::open(ByteStreamIn* stream)
{
...
LAStools::BOOL LASreaderQFITreoffset::open(ByteStreamIn* stream)
{
  if (!LASreaderQFIT::open(stream)) return FALSE;
  // do we need to change anything
...
LAStools::BOOL LASreaderQFITrescalereoffset::open(ByteStreamIn* stream)
{
  if (!LASreaderQFIT::open(stream)) return FALSE;
  // do we need to change anything
  if (scale_factor[0] && (header.x_scale_factor != scale_factor[0]))
