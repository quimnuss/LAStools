...
LAStools::BOOL LASreaderBIN::open(const char* file_name)
{
  if (file_name == 0)
  {
...
LAStools::BOOL LASreaderBIN::open(ByteStreamIn* stream)
{
  int i;
...
LAStools::BOOL LASreaderBIN::seek(const I64 p_index)
{
  if (p_index < npoints)
  {
...
LAStools::BOOL LASreaderBIN::read_point_default()
{
  if (p_count < npoints)
  {
    int echo;
...
void LASreaderBIN::close(LAStools::BOOL close_stream)
{
  if (close_stream)
  {
    if (stream)
...
LAStools::BOOL LASreaderBINrescale::open(ByteStreamIn* stream)
{
...
LAStools::BOOL LASreaderBINreoffset::open(ByteStreamIn* stream)
{
  if (!LASreaderBIN::open(stream)) return FALSE;
  // do we need to change anything
...
LAStools::BOOL LASreaderBINrescalereoffset::open(ByteStreamIn* stream)
{
  if (!LASreaderBIN::open(stream)) return FALSE;
  // do we need to change anything
  if (scale_factor[0] && (header.x_scale_factor != scale_factor[0]))
