...
LAStools::BOOL LASreaderSHP::open(const char* file_name)
{
  if (file_name == 0)
  {
...
LAStools::BOOL LASreaderSHP::seek(const I64 p_index)
{
  return FALSE;
}

LAStools::BOOL LASreaderSHP::read_point_default()
{
...
void LASreaderSHP::close(LAStools::BOOL close_stream)
{
  if (file)
  {
...
LAStools::BOOL LASreaderSHP::reopen(const char* file_name)
{
  if (file_name == 0)
  {
    fprintf(stderr,"ERROR: file name pointer is zero\n");
...
LAStools::BOOL LASreaderSHPrescale::open(const char* file_name)
{
...
LAStools::BOOL LASreaderSHPreoffset::open(const char* file_name)
{
  if (!LASreaderSHP::open(file_name)) return FALSE;
  // do we need to change anything
...
LAStools::BOOL LASreaderSHPrescalereoffset::open(const char* file_name)
{
  if (!LASreaderSHP::open(file_name)) return FALSE;
  // do we need to change anything
  if (scale_factor[0] && (header.x_scale_factor != scale_factor[0]))
